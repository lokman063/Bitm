...
function all(){
    ...
    //selection query
//$query = "SELECT * FROM banners WHERE is_deleted = 0 ORDER BY id DESC ";
...
$query = "SELECT * FROM banners WHERE is_active = 1 ORDER BY id DESC ";
$sth = $this->conn->prepare($query);
$sth->execute();
return $banners = $sth->fetchAll(PDO::FETCH_ASSOC);
}
function inactive(){


$query = "SELECT * FROM banners WHERE is_active = 0 ORDER BY id DESC ";
$sth = $this->conn->prepare($query);
$sth->execute();
return $banners = $sth->fetchAll(PDO::FETCH_ASSOC);
}
...
if (isset($data)){
    ...
    $query = "SELECT * FROM banners WHERE title LIKE '%$searching%' OR link LIKE '%$searching%' ";
